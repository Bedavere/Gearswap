--Originally Built off of spicyryans blu.
-- Rune Management window by eRUPT@asura.
--	Alt-f2 cycles Runes Alt-f1 uses rune.
--
--
...
-- Use seperate precast set when inspire is up
Inspireset = false
...
--	TaeonBody.FC = { name="Taeon Tabard", augments={'Accuracy+23','"Fast Cast"+5','HP+36',} }
	...
	HerculeanGloves.Reso = { name="Herculean Gloves", augments={'Accuracy+7 Attack+3','"Triple Atk."+2','STR+13', } }
	...
	Ogma = {}
	Ogma.TP = { name="Ogma's cape", augments = { 'DEX+20','Accuracy+20 Attack+20','DEX+10','"Dbl.Atk."+10','Damage taken-5%', } }
	...
	Ogma.FC = { name = "Ogma's Cape", augments = { 'HP+60','"Fast Cast"+10','HP+18' } }
	Ogma.Enmity = { name = "Ogma's Cape", augments = { 'Enmity+10', } }
	
------End of Augmented Gear-----------------------------------------------------------------------------------------------------------------------------
	--Idle Sets--
	sets.Idle = {}
	sets.Idle.index = { 'DT', 'Refresh', 'BuffDuration', }
	Idle_ind = 1
	sets.Idle.DT = {
		...
		back		={ name="Ogma's cape", augments={'VIT+20','Eva.+20 /Mag. Eva.+20','VIT+4','Enmity+10','Damage taken-5%',}},
	}
	...
	sets.Idle.BuffDuration = set_combine(sets.Idle.Refresh, {
		ammo		="Staunch Tathlum +1",
		head		="Erilaz Galea +1",
		body		="Runeist Coat +3",
...
-------TP Sets-------------------------------------------
	...
	sets.OneHandedTP.DualWield = {
		...
		legs		={ name="Herculean Trousers", augments={'Accuracy+18 Attack+18','"Triple Atk."+4','Attack+6',}},
		...
		back		={ name="Campestres's Cape", augments={'Pet: Acc.+20 Pet: R.Acc.+20 Pet: Atk.+20 Pet: R.Atk.+20','Eva.+20 /Mag. Eva.+20','Pet: Accuracy+10 Pet: Rng. Acc.+10','"Fast Cast"+10','System: 1 ID: 1246 Val: 4',}},
	}
	...
	sets.OneHandedTP.AccuracyMid = {
		...
		head		={ name="Adhemar Bonnet +1", augments={'DEX+12','AGI+12','Accuracy+20',}},
		body		={ name="Adhemar Jacket +1", augments={'DEX+12','AGI+12','Accuracy+20',}},
		hands		={ name="Herculean Gloves", augments={'Accuracy+25 Attack+25','"Triple Atk."+2','DEX+4','Accuracy+11','Attack+14',}},
		legs		={ name="Herculean Trousers", augments={'Accuracy+18 Attack+18','"Triple Atk."+4','Attack+6',}},
		feet		={ name="Herculean Boots", augments={'Rng.Atk.+18','"Triple Atk."+4','STR+3','Accuracy+10','Attack+4',}},
		neck		="Anu Torque",
		waist		="Sarissapho. Belt",
	...
	sets.TankingTP = {}
	sets.TankingTP.index = { 'Tank','NoParry', 'MEVA' ,'Enmity', 'TankHyb','DDHyb','DDHybAcc'}
...
--		neck = "Orunmila's Torque",
	...
	sets.precast.FastCast.InspireEnhance = set_combine (sets.precast.FastCast.Inspire, {
		hands = "Runeist mitons +2",
		legs = "futhark trousers +1",
		waist = "Siegel sash",
	})
...
RuneDesc = {
	[1] = 'Paralyze / Distract / Ice',
	[2] = 'Silenece Gravity / Wind',
	[3] = 'Stun / Earth',
	[4] = 'Poison / Lightning',
	[5] = 'Slow / Petri / Bind / Water',
	[6] = 'Addle / Amnesia / Fire',
	[7] = 'Blind / Sleep / Dispel / Frazzle / Light',
	[8] = 'Lullaby / Finale / Charm / Dark'
}
...
--               Binds                  --
------------------------------------------
send_command('bind !f1 gs c userune')
send_command('bind !f2 gs c selectrune')
send_command('bind ^f1 gs c toggle Inspire set')
...
send_command('bind ^e input /ja "Elemental Scherzo" <me>')
send_command('bind ^t input /p Odyllic Subterfuge <t>;input /ja "Odyllic Subterfuge" <t>')
...
function self_command(command)
	if command == 'selectrune' then
		cur_rune = cur_rune + 1
		if cur_rune > 8 then
			cur_rune = 1
		end
	elseif command == 'userune' then
		send_command('input /ja "'..RuneList[cur_rune]..'" <me>')
	...
	elseif command == 'toggle Inspire set' then
		if Inspireset then
			Inspireset = false
		else
			Inspireset = true
...
function pc_Magic(spell, act)
	if spell.skill == 'Enhancing Magic' then
		if (buffactive.Valiance or buffactive.Vallation) and Inspireset then
			ChangeGear(sets.precast.FastCast.InspireEnhance)
			msg("Inspire up")
		else
		...
		if (buffactive.Valiance or buffactive.Vallation) and Inspireset then
			ChangeGear(sets.precast.FastCast.Inspire)
			msg("Inspire up")
		else		
...
function mc_Magic(spell, act)
	...
	if BlueMagic_Enmity:contains(spell.english) then
		if BlueMagic_SIR:contains(spell.english) then
			ChangeGear(set_combine(sets.Enmity,sets.SIR))
		else
		ChangeGear(sets.Enmity)
		end
...
function aftercast(spell, act)
	...
	if not spell.interrupted then
		...
		elseif spell.english == "Liement" then
			send_command('wait 170;gs c -cd '..spell.name..': [Ready In 10 Seconds!];wait 10;gs c -cd '..spell.name..': [Ready !]')
		elseif spell.english == "Gambit" then
			send_command('input /p Gambit activated on <t>;wait 62;input /p Gambit 10 seconds left;input /p Gambit off')
		elseif spell.english == "Rayke" then
			send_command('input /p Rayke activated on <t>;wait 20;input /p Rayke 10 seconds left;input /p Rayke off')
		elseif spell.english == "Geist Wall" then
--			windower.send_command("input /p Geist Wall AOE Hate casted successfully")
		elseif spell.english == "Sheep Song" then
			windower.send_command('input /p Sheep Song AOE Hate casted successfully')
		end
	end	
...
gearswap_box = function()
  ...
  str = str..'Tank Mode:	   '
  if TankingTP then
	str = str..'\\cs(0,100,0)On'
  else
	str = str..'\\cs(100,0,0)Off'
  end
  str = str..'\\cr Inspire Set:'
  if Inspireset then
	str = str..'\\cs(0,100,0)On'
  else
	str = str..'\\cs(100,0,0)Off'
  end
  str = str..'\\cr\n    Tank Set:\\cs(0,100,0) '..sets.TankingTP.index[TankingTP_ind]
  str = str..'\\cr'
  str = str..'  Idle:\\cs(0,100,0) '..sets.Idle.index[Idle_ind]
  str = str..'\\cr\n'
  str = str..'    2HTP Set:\\cs(0,100,0)   '..sets.TwoHandedTP.index[TwoHandedTP_ind]
  str = str..'\\cr\n'
  str = str..'Current Rune:\\cs'..RuneColor[cur_rune]..RuneList[cur_rune]
  str = str..'\\cr\n'
  str = str..' Desc:\\cs(0,100,0)'..RuneDesc[cur_rune]
  str = str..'\\cr\n'
...
gearswap_box_config = {pos={x=400,y=300},padding=8,text={font='Segoe UI Symbol',size=12,Fonts={'sans-serif'},},bg={alpha=200},flags={}}
